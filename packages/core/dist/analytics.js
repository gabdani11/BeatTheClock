...
export function calculateStreaks(sessions) {
    const solvedDates = Array.from(new Set(sessions
        .filter(s => s.status === 'solved')
        .map(s => getLocalDateString(s.date)))).sort((a, b) => new Date(b).getTime() - new Date(a).getTime()); // descending (newest first)
...
export function calculateGeneralStats(sessions) {
...
export function calculateTimeAnalytics(sessions) {
    ...
    const avgThinkingTime = Math.round(sessions.reduce((acc, curr) => acc + curr.breakdown.thinkingSeconds, 0) / total);
    const avgCodingTime = Math.round(sessions.reduce((acc, curr) => acc + curr.breakdown.codingSeconds, 0) / total);
    const avgReviewTime = Math.round(sessions.reduce((acc, curr) => acc + curr.breakdown.reviewSeconds, 0) / total);
    const avgPauseCount = Number((sessions.reduce((acc, curr) => acc + curr.pauseCount, 0) / total).toFixed(1));
    let fastestSolve = null;
    let slowestSolve = null;
    if (solved.length > 0) {
...
export function calculateWeeklyProgress(sessions) {
